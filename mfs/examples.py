...
def rectangular_coil_pair():
    ...
    origin = [
        ...
        0,
        0,
    ]  # The origin of the coil pair is located here  note, technically, this is already in the Dashed frame. In this case, it doesn't matter since both systems have [0,0,0] at the same plate
    theta = 0
    phi = 0
    current = 10
    magnetDimensions = {
        ...
        "azDash": 0.2,
        "full spacing": 0.15,
        "spatially distributed": False,
        "axial layers": 10,
        "radial layers": 2,  # Note - we still have to define layers radialy/axially
        "shape": "rect",
        "configuration": "hh",
    }
...
def circular_coil_pair():
    ...
    theta = 10
    phi = 30
    originDash = helpers.rotate_to_dashed_frame(
        np.array(origin), np.radians(theta), np.radians(phi)
    )  # The magnet class expects an origin in the dashed frame; and the "naked" rotation functions expect angles in radians
    current = 10
    magnetDimensions = {
        "radius": 0.1,
        "full spacing": 0.15,
